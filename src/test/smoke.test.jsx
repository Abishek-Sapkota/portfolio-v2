...
test('about renders bio, every skill, all experience and education', () => {
  ...
  profile.experience.forEach(e => expect(screen.getByText(e.company)).toBeInTheDocument())
  profile.education.forEach(e => expect(screen.getByText(e.institution)).toBeInTheDocument())
  expect(screen.getByText(/2\+ years in production/)).toBeInTheDocument()
})
...
test.each(projects.map(p=>[p.slug,p]))('project detail renders the %s case study', (slug, project) => {
  ...
  // Private work projects carry no github_url, so the button is absent by design.
...
test('contact form hands a valid enquiry to a prefilled mailto link', async () => {
  ...
  await user.type(screen.getByPlaceholderText('A backend project'),'API review')
  await user.type(screen.getByPlaceholderText(/What are you building/),'We need help hardening a multi-tenant Django API before launch.')
  ...
  expect(url.searchParams.get('body')).toContain('multi-tenant Django API')
